...
EvaluationReportTemplate **renders** self-contained HTML evaluation reports for VIEWS forecasting models and ensembles from an **injected `EvaluationSource`** — a typed `MetricFrame` per model (ADR-018 / C-108) — rather than acquiring metrics itself at render time. It assembles the comparative metric tables, optionally renders historical-vs-predicted line graphs from on-disk predictions (parquet or numpy PredictionFrame per ADR-012), and delegates all HTML assembly to `ReportModule`. This class does **not** touch WandB: it renders purely from the injected `EvaluationSource` (a `MetricFrame` per model), which the caller (pipeline-core's reporting stage) constructs as an `EvaluationSource` (since pipeline-core 3.3.0 its `PerModelMetricFrameSource`, one `MetricFrameFileSource` per model) reading the persisted frame.
