...
**Entry count:** 91 concerns (69 resolved, 22 open) + 5 disagreements (4 resolved)
...
### C-216: Baseline target vocabulary on views-models development is stale (lr_*_best vs lr_ged_*) and the fixes are stranded on two side branches — frame lookup is exact-name equality, so mismatch alone blanks baseline rows

| Field | Value |
|-------|-------|
| ID | C-216 |
| Tier | 2 — structural fragility with a dated, specific merge hazard; failure is visible-absence (a banner) not a wrong number, but it independently guarantees absent baseline rows even after C-215 is fixed |
| Source | /code-review max (2026-08-17); commits verified in views-models |
| Trigger | When `staging_202608` or `tuning_202607` is merged in either direction (config conflict resolution can silently regress the fixed lineage); or when a development-branch subject evaluating `lr_ged_os`/`lr_ged_ns` next renders a report against baselines still targeting `lr_os_best`/`lr_ns_best` |
| Location | views-models `origin/development`: `models/black_ranger/configs/config_meta.py:13` (`['lr_os_best']`), pink/green_ranger (`['lr_ns_best']`), yellow_ranger (`['lr_os_best']`), white_ranger (targets only in config_hyperparameters as the lr_*_best trio), cm/pgm baseline trios covering only `lr_ged_sb`. Fixes stranded: `ee8b4745`/`d09ee0ad`/`e98c56d3` on `origin/staging_202608` ONLY; second fix lineage `bcbf85e6`/`53ba36b4` on `origin/tuning_202607` ONLY. No PR exists from either branch. |
| Narrative | The fleet's subjects moved to `lr_ged_sb/ns/os`; the baseline models' declared targets never followed. Since the durable path resolves frames by exact directory-name equality (`metricframe_<target>`), a subject asking for black_ranger's `metricframe_lr_ged_os` can never match the only frame black_ranger's eval would write (`metricframe_lr_os_best`). Dev subjects `ravaging_*`/`roaming_*` (targets lr_ged_os/ns) have NO declared baseline covering their targets at all. Dylan diagnosed and fixed this silently ("update qs for baselines", 2026-08-10) but the fix lives on a working branch 48 ahead/36 behind development that also carries unrelated daily loss-tuning — the organizational stuck-fix pattern Simon flagged. Two divergent fix lineages on two branches make the eventual merge a silent-regression hazard. |
| Cross-refs | C-215 (first gate; this is the second), C-217 (third gate), C-219 (declaration layer), Cluster I; views-models branches staging_202608/tuning_202607. |

### C-217: The post-B2 seam is unreachable under the committed pins and the actual machine envs — chungus ensembles pin vpc<3, and the production envs are version-skewed three different ways

| Field | Value |
|-------|-------|
| ID | C-217 |
| Tier | 2 — structural: under committed state the entire evaluation-of-record seam never executes, silently re-entering the retired C-48 WandB-scrape class; failures are absence/skew, not wrong numbers |
| Source | /code-review max (2026-08-17); env evidence gathered on the production machine |
| Trigger | When a chungus/r2darts2 run.sh dependency check next runs (it downgrades the shared env to vpc 2.x per requirements, un-producing frames); or when the vpc pins are bumped to >=3 WITHOUT re-running baseline/constituent evaluations (frames still absent, now via C-215/C-216) |
| Location | views-models `origin/staging_202608`: `ensembles/big_chungus/requirements.txt:1` (also small_chungus, mister_bluesky) pin `views-pipeline-core>=2.0.0,<3.0.0`; views-r2darts2 `pyproject.toml:14` pins the same transitively for all chungus constituents. Machine envs: `envs/views_ensemble` (vpc 3.0.0, NO views_reporting → RuntimeError at report step), `envs/views-baseline` (vpc 2.3.0 against the repo's >=3.0.0 pin), `envs/views-hydranet` (views_evaluation 0.4.0 → producer capability-skip, no frames emitted) |
| Narrative | MetricFrame persistence and the source-consuming reporting stage exist only from vpc 3.0.0 (last 2.x tag 2.3.0 has neither `_save_metric_frame` nor `METRICFRAME_DIR_PREFIX`). The staging runs' committed pins therefore run the OLD WandB-scrape reports — whose stale-run partition trips are documented verbatim in ~14 staging configs as the reason baselines were commented out (see C-219). The observed vanishing is a braid: old-class scrape failures (2.x envs), config omissions, and the new structural defect (3.x envs) — each independently sufficient, which is why it resisted casual diagnosis. This entry owns the pins/envs gate; note it is also Cluster H's pattern (pins qualified against nothing) surfacing INSIDE the platform rather than at PyPI. |
| Cross-refs | C-215/C-216 (co-gates), C-48 ✓ (the failure class the 2.x path re-enters), C-224 (3.0.0-only classification drop), Cluster H (same qualification pathology), Cluster I. |

### C-218: A green pipeline can produce ZERO evaluation reports with no signal — log-only skip on missing subject frame, ignored None return, producer capability-skip, and notifications severed in eight model mains

| Field | Value |
|-------|-------|
| ID | C-218 |
| Tier | 1 — the absence of the deliverable itself is silent end-to-end: exit code 0, no report-space note, no alert; the operator discovers the gap only by listing reports/. This is the silence layer that let C-215/C-216/C-217 run unnoticed for the whole post-B2 period. |
| Source | /code-review max (2026-08-17) |
| Trigger | When an operator next runs `--evaluate --report` in an env whose views_evaluation lacks `to_metric_frame` (capability-skip; envs/views-hydranet today) or whose subject frame is missing (C-215's certain state for 2.x-produced trees) and expects a report to exist |
| Location | vpc 3.0.0 `managers/reporting/stage.py:225` (missing subject frame → log-warning + per-target skip; `generate_evaluation_report` returns None, ignored by every caller); `managers/evaluation/stage.py:404-414` (frame emission capability-skipped when views_evaluation lacks `to_metric_frame`); views-models mains for purple_alien, blazing_meteor, blue_stranger, bold_comet, bright_starship, heavy_freighter, pink_pirate, violet_visitor (e.g. `models/purple_alien/main.py:25`) drop `args.wandb_notifications`, hard-disabling completion/failure alerts |
| Narrative | Three independent silencers stack: the producer may silently not produce (capability-skip), the reporting stage silently skips targets whose frame is missing and its None return is ignored, and the last out-of-band signal (WandB notifications) is wired dead in eight model mains regardless of the CLI flag. Net effect observed on this machine: hydranet-family eval under views_evaluation 0.4.0 succeeds, logs to WandB, emits no frame, generates no report, and announces nothing. Fail-loud doctrine (the views-evaluation ADR-015 epic) exists precisely for this shape; the reporting stage predates it. |
| Cross-refs | C-215/C-216/C-217 (the defects this silence concealed), C-11 (visible-degradation doctrine), views-evaluation ADR-015 (fail-loud precedent), Cluster I. |

### C-219: The baseline declaration/announce layer is silently optional and mislabeled — undeclared baselines are invisible, ~48 config files carry commented-out baseline keys as an institutionalized workaround, and absences are announced as "constituents" with no 0-of-N escalation

| Field | Value |
|-------|-------|
| ID | C-219 |
| Tier | 2 — reports that read as complete while containing zero comparison rows (flagships), plus a signal layer whose vocabulary misdirects diagnosis; wrong-shaped signal rather than wrong numbers |
| Source | /code-review max (2026-08-17); simplification-angle finding on the `constituents` rebinding |
| Trigger | When the next flagship report (rusty_bucket / golden_hour / stellar_horizon — NO `*_baselines` keys on either branch) is read as complete; when a `*_baselines` key is renamed/typoed in views-models (concatenation silently returns `[]`); or when a partition-check failure next prompts an operator to comment out a baseline key instead of fixing the cause (the documented workaround pattern, ~14 staging configs citing red_ranger's stale wandb run) |
| Location | `views_reporting/templates/reports/evaluation.py:194-202` (four `*_baselines` keys enumerated ONLY here; no vpc ConfigurationManager normalization, no validator, no combined_baselines helper; `constituents` rebound at :202 to constituents∪baselines−subject); `:281` (absence banner says "Declared constituent(s)"; `strict_constituents` silently gates baselines and is set by NO views-models config); views-models: 27 staging / 21 development config files with disabled baseline keys; `classification_*_baselines` declared by no config anywhere (dead keys); string-valued key char-splits, None raises TypeError |
| Narrative | Three compounding layers. (1) Declaration is optional and unvalidated: an UNDECLARED baseline produces only a log warning, never an in-report banner — so flagship reports show the subject alone, indistinguishable from a resolution failure. (2) The workaround culture deletes signal at the declaration layer: operators comment out baseline keys to route around the partition guard, each branch carrying a different disabled subset, none reconciled. (3) The announce vocabulary is wrong: baselines merge into `constituents`, so a vanished baseline is reported as a vanished constituent, and the systemic 0-of-N state (every comparator absent — production's actual state since B2) renders identically to one routine absence; readers habituate and diagnose the wrong subsystem. |
| Cross-refs | C-215 (the structural absence this layer failed to surface as systemic), C-216 (drift the comment-outs hide), C-11 (degrade-and-announce doctrine — the announce half is miswired), Cluster I. |

### C-220: Primed post-fix detonation — 17 pgm configs declare the cm baseline trio, and the cross-model consistency guard string-compares level/partition and aborts the WHOLE report on mismatch

| Field | Value |
|-------|-------|
| ID | C-220 |
| Tier | 2 — was dormant only because rows never resolved (C-215, fixed 2026-09-19); now ARMED — it fires the moment a cm-baseline frame exists on disk for a pgm subject that declares it (i.e. after C-223's baseline evaluation run). Registered so the fix sequencing accounts for it. |
| Source | /code-review max (2026-08-17) |
| Trigger | Fixing C-215/C-216/C-217 (baseline rows first resolve) without first scrubbing cross-level baseline declarations and hardening the guard's comparison semantics |
| Location | `views_reporting/templates/reports/evaluation.py:404` (level equality + partition compared as `str(partition_dict[run_type])`, stamped at vpc `managers/evaluation/stage.py:434`; ValueError propagates through the whole report); views-models: 17 pgm-level configs on BOTH branches declaring `average_cmbaseline`/`zero_cmbaseline`/`locf_cmbaseline` (including the pgm baselines declaring their cm counterparts) |
| Narrative | The guard is correct doctrine (one row must not silently compare across levels/partitions) but its blast radius is the whole report and its equality is stringly: semantically identical partitions whose dict repr differs (key order, numpy>=2 scalar reprs, None vs '') compare unequal. Today every such comparison short-circuits at "absent" before reaching the guard; after the resolution fix, skinny_love (pgm) resolving average_cmbaseline (cm frame) converts a vanished ROW into a vanished REPORT. The fix for the vanishing mystery must land WITH per-row degrade-not-abort semantics (or declaration scrubbing) or it trades one incident for a worse one. |
| Cross-refs | C-215/C-216 (whose fix arms this), C-219 (the declarations), C-116 (ambiguity-rendering precedent for degrade semantics), Cluster I. |

### C-221: The MetricFrame corruption path violates the #105/#177 taxonomy end-to-end — non-atomic save, unguarded pre-check that aborts all targets, a retry that mislabels deterministic corruption "transient", and the subject's degraded status discarded into config-blaming notes

| Field | Value |
|-------|-------|
| ID | C-221 |
| Tier | 2 — a single mid-save crash yields either no reports at all or a report whose subject row misdirects to config changes; visible-but-wrong-shaped failure, latent until a crash lands mid-save |
| Source | /code-review max (2026-08-17) |
| Trigger | When a crash/interrupt next lands mid-`MetricFrame.save` (partial metricframe dir on disk) and any report run follows |
| Location | `views_reporting/templates/reports/evaluation.py:220-238` (`_resolve` unconditional immediate retry re-parses deterministically corrupt frames and labels them "degraded — transient"; `:238` discards the subject's own degraded status → renders "not calculated — add <metric> to <cfg_key>" misdirection; `provenance()` at `:84` sits outside the retry); vpc `managers/reporting/stage.py:225` (unguarded pre-check load raises out of the whole multi-target loop); `MetricFrame.save` non-atomic (views-evaluation side) |
| Narrative | Every element of the run-selection era's hard-won taxonomy (visible degrade per unit, loud raise only on integrity, no misdirecting notes) is violated by the corruption path of its replacement. The "add <metric> to config" note is the same misdirection C-48's investigation documented for the scrape era, reborn on the file path. Also duplicates work on the failure path (full parse twice per model per target). Remediation direction: atomic save (stage-then-swap, the views-faoapi C-182 pattern), classify parse errors as degraded-no-retry, guard the pre-check per target, thread the subject's degraded status into an honest note. |
| Cross-refs | C-48 ✓ (the misdirection precedent), C-105-class GitHub #105/#177 (the taxonomy), C-215 (same seam), Cluster I, Cluster F (value-correctness assurance). |

### C-222: All recency/vintage semantics were dropped in the B2 inversion with no replacement — no staleness check on loaded frames, no per-row vintage in the table, no report-of-record marker in reports/

| Field | Value |
|-------|-------|
| ID | C-222 |
| Tier | 2 with a named Tier-1 elevation path: today the harm is diagnostic (scrape-era reports WITH baseline rows sit beside post-B2 ones without, nothing marking which is authoritative — the exact signature Dylan hit); after the resolution fix, a months-stale baseline frame renders as a CURRENT comparison row with no marker — silent wrong-vintage number. Elevate on first observation. |
| Source | /code-review max (2026-08-17) |
| Trigger | When C-215/C-216/C-217 are fixed and an old baseline frame first resolves (stale renders as current); today, whenever anyone sorts reports/ to determine the authoritative evaluation report |
| Location | `views_reporting/sources/metric_frame_file_source.py:43` (loads whatever directory is on disk; no staleness/vintage check vs the old newest-metric-bearing-run selection); `views_reporting/templates/reports/evaluation.py:155-157` (report files accumulate under render-time timestamps, no report-of-record marker); comparison rows carry no per-row run-date/data-version despite frames stamping metadata |
| Narrative | The old resolver's one virtue — recency semantics — was deleted with its vices, and nothing replaced it. C-110's resolution assumed frame provenance closes the stale-vintage class; the provenance is stamped but never READ at render time nor surfaced per row, so the closure is unrealized (dated caveat added to C-110). The report directory itself has no authority marker, which is how pre/post-B2 reports came to disagree silently about whether baselines exist. |
| Cross-refs | C-110 ✓ (resolution caveat added — the stale-relog closure is unrealized), C-34 (provenance stamping this should read), C-112 (staleness-observability sibling), Cluster I, Cluster G (report-of-record is a partner-deliverable property). |

### C-223: Ensemble constituent evaluation short-circuits on cached predictions — constituents' own MetricFrames can be stale or absent while the ensemble pools their cached outputs; baselines have no evaluation orchestrator at all

| Field | Value |
|-------|-------|
| ID | C-223 |
| Tier | 2 — vintage skew between pooled numbers and per-constituent evaluation-of-record, silent under a fixed seam; and nothing anywhere enforces a baseline was ever evaluated at the report's run_type |
| Source | /code-review max (2026-08-17) |
| Trigger | When an ensemble re-run next pools cached constituent predictions (cache invalidation checks sample-count only) while a constituent's evaluation is from another vintage — or when a baseline evaluated only at `calibration` is asked for by a `validation` report (resolves as absent with no distinguishing signal) |
| Location | vpc 3.0.0 `managers/ensemble/prediction_frame_ensemble.py:897` (cached `y_pred.npy` skips the constituent's EvaluationStage subprocess); no baseline-evaluation orchestrator exists in vpc or views-models |
| Narrative | Even under a fully fixed resolution seam, the ensemble's pooled numbers and its constituents' metricframes can come from different runs — the cache gate is sample-count-only, so a config/data change that preserves sample count silently pools new predictions beside old evaluations. Baselines are worse: nothing runs their evaluation on any schedule, so their frames exist only if someone manually evaluated them at the right run_type. The "re-evaluate the baselines" migration step the B2 epic never wrote down (the operational half of this mystery) has no mechanical owner. |
| Cross-refs | C-215 (resolution seam), C-222 (vintage semantics), C-216 (target drift compounds which frames exist), Cluster I. |

### C-224: Installed vpc 3.0.0 drops classification_targets from PFE pooling/evaluation — AP/Brier reports for by_* targets cannot exist under 3.0.0; fixed upstream in 3.0.1, adoption pending in views-models

| Field | Value |
|-------|-------|
| ID | C-224 |
| Tier | 3 — bounded exposure window (their #422 fixed it in 3.0.1; views-models #372 tracks adoption), but while any env pins 3.0.0 the classification half of the report surface silently does not exist, feeding C-218's silence |
| Source | /code-review max sweep (2026-08-17); rusty_bucket's own config comment cites the fix |
| Trigger | While any env/requirements pins `views-pipeline-core==3.0.0` for a classification-target subject (rusty_bucket by_sb/ns/os_best) — until views-models #372's 3.0.1 adoption merges and envs are rebuilt |
| Location | vpc 3.0.0 `managers/ensemble/prediction_frame_ensemble.py:373` (`c.get('targets', c.get('regression_targets', []))` — classification_targets never enters pooling/eval); reporting loop iterates `combined_targets` = regression + classification, hitting the C-218 silent per-target skip for every by_* target |
| Narrative | Under 3.0.0: no pooled by_* predictions, no by_* evaluation, no `metricframe_by_*` dirs → the reporting loop silently skips all three classification targets even with correct pins, env, root, and declarations. Registered (despite the upstream fix) because the exposure is invisible while it lasts and because it demonstrates the C-218 skip-silence converting an upstream bug into a missing deliverable with no signal. |
| Cross-refs | C-218 (the silence that hides it), C-217 (env/pin skew governs who is exposed), views-models #372, vpc #422/3.0.1, Cluster I. |

### C-225: Latent correctness nits in the eval-report path — AP sorted ascending (worst-first), origin-dir discovery crashes on stray dirs, prediction-file regex cannot match the producer's target-suffixed grammar, retired WandB-scrape helper still exported

| Field | Value |
|-------|-------|
| ID | C-225 |
| Tier | 3 — each is real but bounded: wrong ORDERING not wrong values (AP sort), loud crash on an odd directory, a dormant filename-grammar mismatch, and an attractive-nuisance export; none silently corrupts a rendered number today |
| Source | /code-review max below-cap items (2026-08-17), verified by the angles |
| Trigger | AP sort: when classification comparison rows first exist (post C-215/C-224 fixes) — `_maybe_sort`'s fall-through sorts AP ascending, presenting worst-first as if best-first. Regex: when views-models adopts the producer's target-suffixed/underscored-run_type prediction filenames (Xiaolong's vpc#328 threads exactly such suffixes). Origin dirs: when a stray `origin_*` dir (scratch, backup) lands in a predictions tree (`int()` on the split crashes discovery). Scrape helper: when a developer imports the exported dead WandB-scrape helper and reintroduces the retired path. |
| Location | `views_reporting/templates/reports/evaluation.py` (`_maybe_sort` fall-through; `_discover_pf_origins`; prediction parquet regex `^predictions_[^_]+_(\d{8}_\d{6})_(\d+)$`); `views_reporting/sources/__init__.py`-adjacent export of the dead scrape helper |
| Narrative | Four independent below-cap findings bundled at intake (distinct mechanisms, shared locus and tier; split on demand if any is promoted). The AP-sort direction is the one to fix WITH the C-215 fix — it only becomes user-visible when comparison rows exist, and would then mislead model ranking at a glance. |
| Cross-refs | C-215 (fix sequencing), vpc#328 (the grammar that arms the regex mismatch), Cluster I. |

### C-226: Eval-report path hygiene backlog — repeated full loads, per-target rework of target-independent data, heavyweight config access, and structural debt from the review's efficiency/simplification angles

| Field | Value |
|-------|-------|
| ID | C-226 |
| Tier | 4 — cost and legibility, no correctness impact; batched at intake as one backlog entry |
| Source | /code-review max efficiency + simplification angles (2026-08-17) |
| Trigger | The next performance-motivated or refactoring pass over `evaluation.py`/`metric_frame_file_source.py`/vpc's reporting stage — work this list rather than rediscovering it |
| Location / items | Subject MetricFrame fully loaded 3× per target (stage pre-check + provenance() + _resolve; memoize on the source / stat-only has_frame / metadata-only provenance). Target-independent work redone per target: historical parquet re-read (`evaluation.py:486`), prediction-file discovery + 3 sequence-file reads (`:585`), fresh template + ModelPathManager per target (stage.py:232, evaluation.py:477). vpc `_load_historical_data` builds a full ModelManager per constituent just to read config targets (stage.py:277) and full-deserializes the raw parquet per constituent with no column projection or per-path cache (:282). All resolved frames held through the render loop with full-array mask rescans per metric (`:239`; slice to MEAN rows and drop). ~105-line try/except that log-mislabels model reports as "ensemble" and re-raises (`:366`). `eval_types` one-iteration pseudo-state with a lying docstring (`:53`). `_canonical_row`/`_maybe_sort` closures untestable inside a 220-line method (`:303`). Duplicate "Prediction Samples unavailable" emission sites (`:380` vs `:445`). `cell_keys` grid hand-duplicated ~5×. vpc `ReportingContext.entity` dead-but-required field (stage.py:78). |
| Narrative | Recorded verbatim from the verified angle findings so the backlog survives the session; none blocks the mystery fix, several (the memoization, the constituents/baselines naming split at `:202` — see C-219) naturally ride along with it. |
| Cross-refs | C-219 (the naming split), C-215 (ride-along opportunities), Cluster I. |
...
### C-215: Post-B2 cross-model metric resolution is structurally broken — the source is rooted at the SUBJECT's data_generated while the producer writes per-model, and our own seam test pins the defective binding — RESOLVED

| Field | Value |
|-------|-------|
| ID | C-215 |
| Tier | 1 — model-output incorrectness with no error signal in the common case: every post-B2 evaluation report omits ALL baseline/constituent comparison rows; for subjects declaring no baselines the report reads as complete (no banner, log-only warning), and CI is green throughout because every fixture writes multiple models' frames under ONE shared root — a state production never produces. Observed (the baselines-vanishing incident), not latent. |
| Source | /code-review max (baselines-vanishing investigation, 2026-08-17); verdict: the session reconstruction VERIFIED with amendments |
| Trigger | The next `--evaluate --report` run on any subject with declared baselines under vpc>=3.0.0 (rows structurally absent); becomes acutely visible the moment a developer fixes resolution to per-model paths — `tests/test_vpc_seam_contract.py:112` AST-locks `root=<subject's data_generated>` as a LOCKED contract, so the CORRECT fix fails CI |
| Location | vpc 3.0.0 `managers/reporting/stage.py:220` (roots `MetricFrameFileSource` at the subject's own `data/generated`); producer `managers/evaluation/stage.py:439-445` (`_save_metric_frame` writes only under each model's own `data/generated`); our pinning surfaces: `tests/test_vpc_seam_contract.py:112`, `tests/test_eval_report_from_source.py` (dict stub), e2e fixtures, `scripts/generate_demo_eval_reports.py:133` (all share one root) |
| Narrative | The B2 inversion's consumer and producer disagree about WHERE frames live: the reporting stage probes `<subject>/data/generated/<model>/<run_type>/metricframe_<target>` for every comparison model, but nothing ever writes another model's frame under the subject's tree — no copy/symlink mechanism exists anywhere in vpc, views-reporting, or views-models (exhaustively searched); all three ensemble managers share the defect. So `metric_frame(model)` returns None for every baseline/constituent → "absent" → zero comparison rows, regardless of target names, pins, or environment. This is the FIRST gate of the baselines-vanishing braid (C-216/C-217 are independently sufficient co-causes). The assurance layer actively protects the defect: the C-192 seam test locks the subject-rooted binding, and every green suite exercises a shared-root layout production cannot produce — CI proves a working multi-model report while production structurally cannot resolve one (the C-46 false-confidence class, third confirmed firing). Remediation direction (not prescribed here): resolve each comparison model via its OWN path manager (or persist frames to a shared metrics root), then CORRECT the seam-test contract in the same change. |
| Cross-refs | C-48/C-108 ✓ (their "end-to-end complete" resolutions now carry dated correction notes pointing here); C-192 ✓ (the seam test that pins the wrong contract); C-46 (CI-green-while-production-broken sibling); C-218 (the silence layer that let this run unnoticed); C-219 (declaration layer); Cluster I (root); views-r2darts2#23 / Slack (Dylan's observed symptom). |
| Resolved | 2026-09-19 (vpc 3.3.0 on PyPI + our #287 seam correction, same window) |
| Resolution | **Both halves of the seam landed in step, as the C-192 correction note required** (the separate C-220/C-225 constraint was not — see Cluster I). Producer/consumer agreement: views-pipeline-core 3.3.0's reporting stage now constructs `PerModelMetricFrameSource` — one of OUR `MetricFrameFileSource`s per model, each rooted at THAT model's own `data/generated` (vpc #485, PR #522, their C-328) — so baseline/constituent frames are looked up where the producer writes them. Our locked layout `root/<model>/<run_type>/metricframe_<target>` is untouched; only the root per model changed. Our half: the seam test that had faithfully locked the defective subject-rooted call (`test_stage_constructs_source_with_our_init_contract`) is replaced by two — one pins the new construction (`primary_root=<subject>.data_generated`), one is a REAL on-disk round-trip (frames saved under two different roots, resolved via the public `metric_frame()` of the stage's source CLASS with an injected test resolver (production's default `model_data_generated` resolver is exercised by vpc's own C-328 tests, not here), values read back exactly; the subject-rooted shape provably returns None for the other model) — and test 1 also asserts NO `root_of=` override in the stage call, so the C-215 shape cannot be re-expressed through the new class. The shared-root fixtures elsewhere (`test_e2e_eval_report.py`, `scripts/generate_demo_eval_reports.py`) still exist and are now the residual of this blind spot, not its closure. Pin raised to `views-pipeline-core>=3.3.0` (a <3.3.0 stage would silently blank rows again); suite green against the RELEASED 3.3.0 (443). No vpc ADR records this decision yet (their ADR-062 is unrelated — retire-by-refusal); the per-model rule exists as vpc #485 + PR #522 + C-328 and our draft ADR text of 2026-08-21, which should now be filed as the record. **What this does NOT do:** frames still only exist for models that were actually evaluated at the report's run_type (C-223 — the one-time baseline re-evaluation remains an operational step), C-216's config drift and C-217's pins on views-models are unchanged, and the announce/vintage layers (C-219/C-222) are still open in #287 items 2-4. The end-to-end proof — a real ensemble report with baseline rows — is still owed and depends on those. |

### C-227: CM choropleth silently draws the DEAD country when a frame carries two country_ids under one ISO code — RESOLVED

| Field | Value |
|-------|-------|
| ID | C-227 |
| Tier | 2 — silent-wrong-map class (a value drawn over the wrong polygon with no signal), but LATENT: every real FORECAST fixture carries exactly ONE member of each dead/live pair (verified 2026-09-17; the historical fixtures carry both, but never reach the map). **Elevate to Tier 1 on the first observed dead-over-live draw.** |
| Source | Issue #290 (filed from the pipeline-core side while investigating vpc#509); mechanism reproduced empirically this session |
| Trigger | When a producer emits a dissolved entity beside its successor in one CM frame — views-r2darts2 ≥0.2.0 forecasts every entity ever seen (r2darts2#49); pipeline-core 3.3.0 ADR-064 now refuses such predictions at ITS boundary, but nothing at OUR boundary did until this fix. Also: any future producer/aggregator that keys rows by something other than the live `country_id`. |
| Location | `views_reporting/mapping/mapping.py::build_mapping_dataframe` CM branch (the single chokepoint; merge `left_on="isoab", right_on="ADM0_A3"`) → `_plot_interactive_map` `pivot_table(index=ADM0_A3, aggfunc="first")` + `drop_duplicates(ADM0_A3)` for hover props, and `_plot_static_map`'s overpaint (last drawn wins). Data: `views_reporting/metadata/data/country.parquet` — **11 duplicate-isoab groups / 27 rows** (issue #290 counted 4 in the CM panel; the file also has SUN×6, YEM×3, YUG, DEU, ETH, SAU, ZAF); 9 of the 11 join the shapefile (SUN/YUG sit in the C-206 retired allowlist). |
| Narrative | The bundled metadata legitimately carries dead/live pairs sharing an ISO code (Sudan 59 pre-2011 vs 245, Serbia 230/233, Tanzania 236/242, Indonesia 208/209 …). If a frame carries BOTH members, the merge inflates to two rows per polygon and every downstream resolution site picks silently: reproduced with `[59, 245]` — the pivot drew the dead Sudan's value for SDN and the hover reported country_id 59. Row order is frame order = ascending `country_id`, so the DEAD id wins in 4 of the 9 joinable pairs (IDN, SDN, SRB, TZA — the four #290 counted) and the live one happens to win in the other 5 (DEU, ETH, SAU, YEM, ZAF); the pick is arbitrary in all nine. The shapefile side is fan-out-safe (177 unique ADM0_A3 post-C-206), so inflation can only come from the frame. The PGM crosswalk resolves the same 11 groups at build time by #242's most-recent-observation rule (C-211); render time has no month-of-last-observation to apply it, so it can only DETECT. Unlike ragged coverage (C-111, legitimate, warns once) a FORECAST frame never legitimately carries both — its entity set is fixed at the origin — so this raises (ADR-008, with the required ERROR log), naming each collided code and its ids. Per CODE rather than per (code, month), deliberately: the repo's own historical fixture shows real transitions put both ids in the same month (Sudan 59+245 at 379, Indonesia 208+209 at 269), so per-month would not rescue them, and the hover props (`drop_duplicates(ADM0_A3)`) key on code alone. Mirror image of C-206: that was the MISSING-row failure at this merge line, this is the EXTRA-row one. |
| Cross-refs | C-206 (same merge line, opposite failure — omission), C-211 (the coding transition; the PGM crosswalk's #242 collision rule), C-111 (coverage-check precedent; why this raises instead of warns), C-190 (omission-reads-as-no-risk sibling), C-212 (guard uses the two index columns only), Cluster F; r2darts2#49 / vpc ADR-064 (the producers and their upstream refusal); #290. |
| Resolved | 2026-09-17 (#290 fix PR) |
| Resolution | Guard at the CM chokepoint, immediately after `__check_missing_geometries` (rows without geometry don't count) and before `self._mapping_dataframe` is set: `groupby(ADM0_A3)[country_id].nunique() > 1` → `ValueError` naming each collided ISO with its sorted country_ids and the register ref — "no silent country picking". RED→GREEN in `tests/test_mapping_characterization.py::TestDuplicateIsoIsLoud` against the REAL bundle (no label mock — both 59 and 245 resolve to SDN there): `[59, 245]` raises naming both ids; `[245]` alone renders one SDN row. PGM raster/image sites untouched (unreachable at CM; `gid` keys are unique by construction). **First catch was our own test double:** the 191-country fixture e2e tests labelled the map through `mock_labels_for_index`, a positional double cycling 10 ISO codes — ~19 country_ids per code, a collision real data never has, which the silent pick had been absorbing. Those tests now use the REAL bundled metadata (offline since C-22; zero collisions on the fixture); the synthetic e2e tests keep the double (3-4 countries). Residual: detection only — resolution belongs to the producer (ADR-064). **Scope residual:** the CM map today renders only forecast frames (`forecast.py`); if a future feature maps a historical/actuals frame spanning a country-system change, this guard refuses it with a producer-blaming message that is the wrong diagnosis for that shape — such a map needs a month-aware rule (C-211's #242 most-recent-observation) before it can exist. Also noted: `tests/conftest.py::mock_isoab_for_index` (which `mock_labels_for_index` delegates to) cycles 10 codes positionally and now carries a docstring warning that >10 entities trip this guard. |
...
| Resolution | **`tests/test_vpc_seam_contract.py` executes against the REAL pipeline-core stage** (skip-if-absent, C-46 contract): AST-pins the stage's single `evaluation_template.generate(source=, target=)` call (keyword-only, exactly those two); binds our `generate` signature against that call; pins the `MetricFrameFileSource(...)` construction kwargs + `root=<data_generated>`; pins the on-disk layout by **executable equality against the producer's own exported `METRICFRAME_DIR_PREFIX`** constant; bonus: both `forecast_template.generate(...)` call sites bind our forecast signature. RED demonstrated for a signature rename and a path-suffix drift. Rider: the stale "provisional layout" docstring in `metric_frame_file_source.py` corrected to the locked-contract language (the producer had long declared it C-202-locked). **CORRECTION NOTE (2026-08-17, C-215): the AST pin faithfully locks the REAL stage call — but the real call is defective (subject-rooted source vs per-model producer), so this test now PROTECTS the defect: the correct per-model fix fails this test. When C-215 is fixed, the locked contract must be corrected in the same change, not worked around.** **CLOSED 2026-09-19: corrected in the same window as vpc 3.3.0 — the test now pins `PerModelMetricFrameSource(primary_root=<subject>.data_generated)` and a real per-model-layout fixture (see C-215 Resolution).** |
...
| Resolved | 2026-06-27 (B2 / C-108) |
| Resolution | Resolved 2026-06-27 (B2 / C-108). The two-sources-of-truth root is gone: the report reads the typed `MetricFrame` evaluation-of-record (persisted by pipeline-core), not the WandB cloud replica. The `get_latest_run`/`format_evaluation_dict` scrape and the `evaluation_run_resolver` selection seam are deleted, so the entire #105/#106/#177 run-selection failure class is structurally removed — there is no render-time run selection at all. **CORRECTION NOTE (2026-08-17, /code-review max — C-215):** the replacement path's PRODUCTION binding is broken — the reporting stage roots the file source at the subject's own tree while frames persist per-model, so the read-local fix has never actually resolved a cross-model row in production; the "constituents on different machines" caveat this entry recorded as the reason the cloud fetch existed was never answered, only deleted. The resolution of the two-sources-of-truth root STANDS; the delivery of its replacement does not. See C-215/C-222. |
...
| Resolved | 2026-06-27 (B2 / C-108) |
| Resolution | Resolved 2026-06-27 (B2). **CORRECTION NOTE (2026-08-17, C-215): "complete end-to-end" is true of the ARCHITECTURE (port, producer, caller switch all exist) but not of production behaviour — the caller roots the source at the subject's tree while the producer writes per-model, so cross-model resolution has never worked outside shared-root test fixtures. The inversion stands; its production wiring is C-215.** The dependency inversion is complete end-to-end on the evaluation side. B1 (#173) introduced the injected `EvaluationSource` port + `MetricFrameFileSource`/`mean_metric_value`; pipeline-core shipped the producer, persistence, and caller switch (evaluation-of-record epic #224, PRs #249–253) wiring its reporting stage to `generate(source=MetricFrameFileSource(...), target=...)`; B2 deletes the interim WandB scrape (`wandb_evaluation_source.py` + `evaluation_run_resolver.py`), leaving `generate(self, source, target)` as pure render-from-given-data. The eval path imports no wandb (verified `git grep`). C-27/C-48/C-110 (eval-side symptoms) resolve with it. Cluster A's non-eval members — C-22 (viewser), C-114 (private pipeline-core internals) — remain open; Cluster A is no longer rooted on the eval inversion. |
