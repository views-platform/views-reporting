...
**Entry count:** 78 concerns (63 resolved, 15 open) + 5 disagreements (4 resolved)
...
### C-214: Plotly-injection and single-library guards key on literal plotly-internal strings — a plotly bump can strip the JS from delivered reports
...
### C-213: Coordinated-release coupling to views-pipeline-core — the `>=3.0.0` pin outruns PyPI (2.3.0) and the consumed import paths are un-qualified against vpc's shim removals
