...
**Entry count:** 73 concerns (56 resolved, 17 open) + 5 disagreements (4 resolved)
...
### C-208: Gapped PGM lattices render misplaced/stretched cells — heatmap midpoint-stretch (live) and PNG even-spread (latent)
...
### C-209: Raster budget guard measures data rows, decoupled from the rendered z payload — breaks the C-203 contract under any dense-lattice fix
...
| Cross-refs | C-26 / C-203 (RESOLVED — the budget lineage this recalibrates); C-208 (the geometry fix that makes this acute — land together); C-38 (sibling artifact-size growth mechanism); ADR-008/ADR-016 (fail-loud at the Compose boundary); Cluster C (PRIO-GRID scale discipline). |
