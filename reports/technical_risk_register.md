...
**Entry count:** 91 concerns (68 resolved, 23 open) + 5 disagreements (4 resolved)
...
### C-215: Post-B2 cross-model metric resolution is structurally broken — the source is rooted at the SUBJECT's data_generated while the producer writes per-model, and our own seam test pins the defective binding
...
|-------|-------|
| ID | C-220 |
| Tier | 2 — dormant today ONLY because rows never resolve (C-215); becomes an acute whole-report-loss failure the moment the resolution defects are fixed. Registered now so the fix sequencing accounts for it. |
| Source | /code-review max (2026-08-17) |
