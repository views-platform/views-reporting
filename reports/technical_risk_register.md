...
**Entry count:** 69 concerns (49 resolved, 20 open) + 5 disagreements (4 resolved)
...
### C-192: No executable cross-repo contract test for the eval `generate(source, target)` / MetricFrame consumption (views-reporting ↔ pipeline-core)
...
### C-205: Global PGM raster map has no geographic reference (no coastlines/borders) and equirectangular distortion
...
| Trigger | When a **whole-globe** PGM forecast map (post Africa+ME expansion) is rendered as the raster heatmap and a human tries to *orient* on it — locate a country/region by eye. |
| Location | `views_reporting/mapping/mapping.py` `_plot_interactive_raster_map` (`go.Heatmap` over raw lon/lat; no coastline/border layer); the choropleth path at least implied coastlines via cell polygons. |
| Narrative | The raster renders values on a bare `(lon, lat)` lattice — no coastlines, no national borders — and equirectangular lon/lat distorts area toward the poles. For the current Africa+ME subset this is tolerable (the shape is recognisable), but at **global** scale a floating value-field with no geography is hard to eyeball, undercutting the whole point of the map ("locate the forecast"). Remediation: overlay a **kilobyte-scale coastline/border line layer** (e.g. Natural Earth 110m lines as a thin scatter/line trace — NOT the 56 MB PRIO-GRID cell shapefile, C-23) so the global raster is orientable; consider an explicit projection note. Pairs with the PNG/`go.Image` globe-scale render (the durable large-grid path) — do them together as globe-expansion readiness. Open / tracked. |
| Cross-refs | C-26 (the raster path this annotates); C-23 (the heavy cell shapefile to avoid reusing); C-191 (colour legibility — already handled); the tracked PNG/image globe-scale fallback. |
...
| Cross-refs | Extraction postmortem (C-105/C-106 in pipeline-core); C-23 (the 56 MB shapefile feeds the choropleth path); #118 (the fail-loud guard); C-189/C-190/C-191 (the #125 large-render methodology guards); C-109 (point-summary/uncertainty); ADR-016 (config injected to the Render layer); ADR-008 (fail-loud) |
