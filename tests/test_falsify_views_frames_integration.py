"""Falsification probes — claim: "views-frames is 100% integrated (development)".

Generated by the /falsify audit (2026-06-24) as xfail stubs documenting the
then-open gaps. Each test encodes the **strong** reading of "100% integrated":
that *every* data path in views-reporting consumes views-frames as its contract,
with no residual pipeline-core dataset / torch / wandb coupling.

ALL PROBES NOW PASS (the last xfail — P6, the entity_metadata private type-hints
— was promoted 2026-07-02 with the C-114 dead-accessor deletion). The tests
remain as permanent guards locking the integration in; each probe's comment
records which register entry / PR closed its gap.
"""
...
def test_p1_statistics_api_is_views_frames_only():
    ...
    # on the stats path. (entity_metadata.py's private type-hints are gone too —
    # deleted with the dead dataset-parameter accessors; see P6.)
...
def test_p6_no_pipeline_core_private_internal_imports():
    """P6 — RESOLVED (C-114): the last holdout (`metadata/entity_metadata.py`'s
    TYPE_CHECKING hints on its dataset-keyed accessors) was deleted with the dead
    dataset-parameter accessor surface — nothing on the platform called it, so it
    did not need to wait for the C-22 viewser sprint. Now a permanent guard
    against reintroducing pipeline-core private-internal imports."""
